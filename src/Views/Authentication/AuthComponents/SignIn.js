    ...
    const login = async () => {
        try {
            ...
            if (!user.email || !user.password) {
                alert("Field Can not be blanked")
                return
            }
        ...
        <div className="flex w-full h-full justify-center flex-col gap-5">
            ...
            <div className="w-4/5 mx-auto flex flex-col gap-4">
                ...
                <div className="text-center">
                    <button onClick={login} className="w-36 h-10 text-xl font-bold rounded-3xl text-white bg-[rgba(86,112,235,1)] hover:bg-[#465abe] transition-all duration-500 ease-linear">Login</button>
                </div>
            </div>
        </div>
    )
